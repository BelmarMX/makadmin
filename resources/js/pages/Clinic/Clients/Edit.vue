...
const avatarPreview = ref<string | null>(props.client.avatar ?? null);
...
const form = useForm<{
    ...
    phone_alt: props.client.phone_alt ?? '',
    avatar: null,
...
function submit(): void {
    form
        ...
        .post(clientRoutes.update({ clinic, client: props.client.id }).url, {
            forceFormData: true,
...
<template>
    ...
    <form class="space-y-6" @submit.prevent="submit">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-foreground">Editar tutor</h1>
                <p class="text-sm text-muted-foreground">{{ client.name }}</p>
            </div>
            <div class="flex gap-2">
                <Button variant="outline" as-child v-tooltip.bottom="'Volver al detalle'">
