...
const preview = ref<string | null>(props.patient.photo_url ?? null);
...
const form = useForm<{
    ...
    species_id: props.patient.species_id ?? null,
    breed_id: props.patient.breed_id ?? null,
    temperament_id: props.patient.temperament_id ?? null,
    coat_color_id: props.patient.coat_color_id ?? null,
    birth_date: props.patient.birth_date ?? '',
    microchip: props.patient.microchip ?? '',
    size: props.patient.size ?? null,
    weight_kg: props.patient.weight_kg ? String(props.patient.weight_kg) : '',
    notes: props.patient.notes ?? '',
    is_sterilized: props.patient.is_sterilized,
    is_deceased: props.patient.is_deceased,
    deceased_at: props.patient.deceased_at ?? '',
    photo: null,
});
...
const deceasedAtModel = computed<Date | null>({
    get: () => parseDate(form.deceased_at),
    set: (value) => {
...
function submit(): void {
    form.transform((data) => ({ ...data, _method: 'PUT' })).post(
        patientRoutes.update({ clinic, patient: props.patient.id }).url,
        {
            forceFormData: true,
            onSuccess: () => toast.success('Paciente actualizado'),
...
<template>
    ...
    <form class="space-y-6" @submit.prevent="submit">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-foreground">Editar paciente</h1>
                <p class="text-sm text-muted-foreground">{{ patient.name }}</p>
            </div>
            <div class="flex gap-2">
                <Button variant="outline" as-child v-tooltip.bottom="'Volver a la ficha'">
                    <Link :href="patientRoutes.show({ clinic, patient: patient.id }).url">
        ...
        <div class="grid gap-4 md:grid-cols-[12rem_1fr]">
            ...
            <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                ...
                <div class="grid gap-1">
                    <FloatLabel variant="on">
                        <Select v-model="form.sex" :options="sexOptions" option-label="label" option-value="value" input-id="sex" class="w-full" />
                ...
                <div class="grid gap-1">
                    <div class="flex gap-2">
                        <div class="min-w-0 flex-1">
                            <FloatLabel variant="on">
                                <Select v-model="form.breed_id" :options="filteredBreeds" option-label="name" option-value="id" input-id="breed" class="w-full" :disabled="!form.species_id" />
                ...
                <div class="grid gap-1">
                    <div class="flex gap-2">
                        <div class="min-w-0 flex-1">
                            <FloatLabel variant="on">
                                <Select v-model="form.temperament_id" :options="temperaments" option-label="name" option-value="id" input-id="temperament" class="w-full" />
                ...
                <div class="grid gap-1">
                    <FloatLabel variant="on">
                        <Select v-model="form.size" :options="sizeOptions" option-label="label" option-value="value" input-id="size" class="w-full" />
