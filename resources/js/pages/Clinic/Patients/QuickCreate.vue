...
import ClientQuickSearch from '@/components/domain/Patient/ClientQuickSearch.vue';
...
const selectedClient = ref<{ id: number; name: string } | null>(props.client);
...
function submit(): void {
    form.post(patientRoutes.quickStore(clinic).url, {
        forceFormData: true,
...
<template>
    <Head title="Alta rápida de pacientes" />

    <form class="space-y-6" @submit.prevent="submit">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-foreground">Alta rápida de pacientes</h1>
                <p class="text-sm text-muted-foreground">Selecciona tutor y registra la mascota en un solo paso.</p>
            </div>
        ...
        <div class="grid gap-1">
            <FloatLabel variant="on">
                ...
                />
                <label for="client_id">Tutor</label>
            </FloatLabel>
            <InputError :message="form.errors.client_id" />
            <p v-if="selectedClient" class="text-sm text-muted-foreground">Tutor seleccionado: {{ selectedClient.name }}</p>
        </div>
